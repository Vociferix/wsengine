/* wsengine.c
 *
 * stdio-based API for Wireshark,
 * by Jack Bernard <jack.a.bernard.jr@gmail.com>.
 *
...
int main(int argc, char *argv[]) {
  ...
  cmdarg_err_init(wse_cmdarg_err, wse_cmdarg_err_cont);

  /* Initialize log handler early so we can have proper logging during startup. */
  ws_log_init("wsengine", vcmdarg_err);
  ...
  if (init_progfile_dir_error != NULL) {
    fprintf(stderr, "wsengine: Can't get pathname of wsengine program: %s.\n",
            init_progfile_dir_error);
  }

  /* Initialize the version information. */
  ws_init_version_info("WSEngine (Wireshark)", NULL,
                       epan_get_compiled_version_info,
                       epan_get_runtime_version_info);

  init_report_message("wsengine", &wse_report_routines);

  // XXX: sharkd does some initialization here
  ...
  if (argc > 1) {
      ...
      if (!read_cmd(cr, &cmd)) {
          fprintf(stderr, "wsengine: failed to read and parse command\n");
          goto clean_exit;
      }

      if (!cmd_obj_get_string(&cmd, "mode", &mode)) {
          fprintf(stderr, "wsengine: expected command mode\n");
          goto clean_exit;
      }
  }
...
wse_get_frame_ts(struct packet_provider_data *prov, guint32 frame_num)
{
 ...
 */
static void
wse_cmdarg_err(const char *msg_format, va_list ap)
{
  fprintf(stderr, "wsengine: ");
 ...
 */
static void
wse_cmdarg_err_cont(const char *msg_format, va_list ap)
{
  vfprintf(stderr, msg_format, ap);
  fprintf(stderr, "\n");
}
...
cf_status_t
wse_cf_open(const char *fname, unsigned int type, gboolean is_tempfile, int *err)
{
  return cf_open(&cfile, fname, type, is_tempfile, err);
}

int
wse_load_cap_file(void)
{
  return load_cap_file(&cfile, 0, 0);
}

frame_data *
wse_get_frame(guint32 framenum)
{
                       ...
                       int *err, gchar **err_info)
{
  ...
  create_proto_tree = ((dissect_flags & WSE_DISSECT_FLAG_PROTO_TREE) ||
                      ((dissect_flags & WSE_DISSECT_FLAG_COLOR) && color_filters_used()) ||
                      (cinfo && have_custom_cols(cinfo)));
  epan_dissect_init(&edt, cfile.epan, create_proto_tree, (dissect_flags & WSE_DISSECT_FLAG_PROTO_TREE));

  if (dissect_flags & WSE_DISSECT_FLAG_COLOR) {
  ...
  cb(&edt, (dissect_flags & WSE_DISSECT_FLAG_PROTO_TREE) ? edt.tree : NULL,
     cinfo, (dissect_flags & WSE_DISSECT_FLAG_BYTES) ? edt.pi.data_src : NULL,
     data);
...
int
wse_filter(const char *dftext, guint8 **result)
{
  ...
  for (framenum = 1; framenum <= frames_count; framenum++) {
    frame_data *fdata = wse_get_frame(framenum);
 ...
 */
wtap_block_t
wse_get_modified_block(const frame_data *fd)
{
 ...
 */
wtap_block_t
wse_get_packet_block(const frame_data *fd)
{
...
int
wse_set_modified_block(frame_data *fd, wtap_block_t new_block)
{
